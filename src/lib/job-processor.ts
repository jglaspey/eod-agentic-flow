/**
 * Job Processing Utility - Extracted from process route to avoid circular imports
 * Contains the core processing logic used by both synchronous and queued processing
 */
...
/**
 * Process files using the existing agent pipeline
 * Extracted from route.ts to be used by both sync and async processing
 */
export async function processFilesWithNewAgent(
  jobId: string,
  estimateFile: File,
