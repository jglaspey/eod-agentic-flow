...
describe('Business Rules Engine', () => {
  ...
  describe('BusinessRulesEngine', () => {
    ...
    test('should return available rules for inspection', () => {
      ...
      expect(rules.map(r => r.ruleId)).toContain('hip-ridge-cap-quality')
      expect(rules.map(r => r.ruleId)).toContain('starter-row-quality')
      expect(rules.map(r => r.ruleId)).toContain('drip-edge-gutter-coverage')
      expect(rules.map(r => r.ruleId)).toContain('ice-water-barrier-code')
    })
  })
})